...
export class EmployeeService {
  ...
  constructor(private http: HttpClient,  private storedEmployeeUrl: CookieService){
  ...
  addEmployee(requestBody: any): Observable<any> {
    ...
    return this.http.post<any>(
      this.url + '/v1/api/employee',body,{headers: headers} );
  }
  ...
  getLoggedHoursForDev(empId: string): Observable<any[]> {
    return this.http.get<any[]>(
      this.url + "projectlogged/dev/" + empId
    );
  }
